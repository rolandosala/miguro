<template>
    <div class=" offcanvas offcanvas-start" tabindex="-1" id="menusidebar" aria-labelledby="offcanvasExampleLabel">
        <div class=" col-sm-12 ">
            <button type="button" class="btn-close p-3 text-center" data-bs-dismiss="offcanvas"
                aria-label="Close"></button>
            <div class="col-sm-12 position-sticky top-0 z-3 text-center p-4 ">

                <a href="#" class="text-center text-dark text-decoration-none ">
                    <span class="fs-1 fw-bold">MIGURO</span>
                </a>
                <hr>
            </div>
        ...
        <div class="offcanvas-body">
            <ul class="nav nav-pills mb-auto d-flex flex-column">
                <li v-for="data in menu_1">
                    <RouterLink :to="data.link">
                        <span class="nav-link link-dark mx-1 menu fs-4" :class="{ active: $route.path === data.link }"
                            :data-bs-toggle="data.collapse" :href="'#' + data.target" role="button">{{ data.item
                            }}</span>
                    </RouterLink>
                    <div :class="data.collapse" class="accordion-collapse" :id="data.target">
                        <ul class="nav nav-pills flex-column mx-3 text-left">
                            <li v-for="data in data.submenu" class="submenu">
                                <RouterLink :to="data.link">
                                    <a href="#" class="nav-link link-dark mx-1 fs-5"
            ...
            <ul class="nav nav-pills flex-column mb-auto text-left">
                <li v-for="data in menu_2">
                    <RouterLink :to="data.link">
                        <span class="nav-link link-dark mx-1 fs-4" :data-bs-toggle="data.collapse" :href="'#' + data.target"
            ...
            <hr>
            <button class="btn btn-outline-success form-control fs-4" data-bs-toggle="offcanvas"
                data-bs-target="#chatwithmiguroCanvas" aria-controls="offcanvasResponsive">Chat with Miguro</button>
            <button class="btn btn-outline-success mt-2 form-control fs-4" data-bs-toggle="offcanvas"
...
export default {
    data() {
        return {
            ...
            menu_2: [
                {
                    item: 'Practice', link: '', collapse: 'collapse', target: 'practice',
                    submenu: [
                        { item: 'Kanji Flashcard', link: '/flashcards' },
